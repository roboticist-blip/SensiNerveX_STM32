...
static uint32_t s_imu_sample_count  = 0UL; /* Number of raw 100 Hz IMU samples taken */

/* Most recent model prediction, updated once per feature window (1 Hz)
 * in Mod_FeatureWindow_Tick. Mod_ImuSample_Tick (100 Hz) tags every raw
 * IMU row with this same value between window updates — see the
 * IMPORTANT note on DataLogger_LogRawIMU() in DataLogger.h for why this
 * is NOT a fresh per-sample prediction. */
...
static void Mod_ImuSample_Tick(void)
{
    ...
    /* Persist the raw 6-axis sensor reading to the IMU file — non-blocking,
     * queued (see DataLogger.h / DataLogger.c). This is the actual 100 Hz
     * ground truth: everything downstream (feature vectors, predictions)
     * is derived from this stream, so it's what validates the
     * complementary filter and feature extraction rather than just
     * re-describing their own output. t_start (already read above)
     * doubles as this sample's timestamp — one fewer timer read.
     * s_latest_pred is the most recent WINDOW prediction (1 Hz), not a
     * fresh per-sample one — see the note on DataLogger_LogRawIMU(). */
...
static void Mod_FeatureWindow_Tick(void)
{
    ...
    /* Update the value Mod_ImuSample_Tick tags every raw IMU row with,
